...
// --- Pure validation function (reusable) ---
...
export function runValidationChecks(args: ValidationInput): ValidationOutput {
  ...
  const [minPrice, maxPrice] = priceBounds[category] || [0.1, 100];
  if (
    ...
    reasons.push(
      `Shelf price $${args.shelf_price} outside typical ${category} range ($${minPrice}-$${maxPrice})`
    );
  ...
  // Rating bounds check
  if (args.rating != null && (args.rating < 0 || args.rating > 5)) {
    reasons.push(`Rating ${args.rating} outside valid range 0-5`);
    if (severity !== 'fail') severity = 'warn';
  }

  // Confidence bounds check
  if (args.confidence != null && (args.confidence < 0 || args.confidence > 1)) {
    reasons.push(`Confidence ${args.confidence} outside valid range 0-1`);
    if (severity !== 'fail') severity = 'warn';
  }
...
// --- Tool wrapper (kept for optional standalone pre-checks) ---

export const validateObservationTool = tool(
  'validate_observation',
  'Optional pre-check: run quality checks on observation data. Note: write_observation auto-validates, so you only need this for dry-run checks before writing.',
  {
  ...
  async (args) => {
    const result = runValidationChecks(args);
    return {
      content: [
        {
          type: 'text' as const,
